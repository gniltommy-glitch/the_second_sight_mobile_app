...
class _HomeState extends State<Home> {
  ...
  Future<void> findPlaces() async {
    ...
    await s.perform(() async {
      ...
      if (found.isEmpty) {
        s.notice =
            'Không tìm thấy địa điểm. Thử địa chỉ cụ thể hơn hoặc chọn trên bản đồ.';
      }
    });
            ...
            children: [
              ...
              OutlinedButton.icon(
                ...
                onPressed: () => s.perform(() async {
                  ...
                  await shareText(
                    '${s.demo ? '[MÔ PHỎNG — KHÔNG PHẢI SOS THẬT] ' : ''}Tôi cần hỗ trợ. '
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [
        TextButton.icon(
          onPressed: sos,
          icon: const Icon(Icons.sos, color: Color(0xFFB53E32)),
          label: const Text(
            'SOS',
            style: TextStyle(
              color: Color(0xFFB53E32),
    ...
    body: SafeArea(
      child: Column(
        children: [
          if (s.demo)
            banner(
              'CHẾ ĐỘ MÔ PHỎNG • Không dùng để đi ngoài đường',
          ...
          if (s.hazard != null &&
            ...
            Semantics(
              liveRegion: true,
              child: banner(s.hazard!, const Color(0xFFFFDAD6)),
            ),
          Expanded(
            child: switch (tab) {
              0 => navigatePage(),
  ...
  Widget banner(String text, Color color, {VoidCallback? onClose}) => Container(
  ...
  Widget navigatePage() => ListView(
    ...
    children: [
      ...
      Text(
        'Điện thoại dẫn đường. Kính quan sát xung quanh.',
      ...
      Row(
        children: [
          ...
          const SizedBox(width: 8),
          IconButton.filled(
            onPressed: searching ? null : findPlaces,
            tooltip: 'Tìm kiếm',
            ...
            children: s.favorites
                .map(
                  (p) => InputChip(
                    ...
                    label: Text(p.name),
                    onPressed: () => select(p),
                    onDeleted: () => s.perform(() => s.removeFavorite(p)),
                  ),
                )
                .toList(),
          ),
            ...
            children: [
              FlutterMap(
                ...
                options: MapOptions(
                  ...
                  initialZoom: 15,
                  onLongPress: (_, point) =>
                      select(Place('Điểm đã chọn trên bản đồ', point)),
                ),
                children: [
                  TileLayer(
                    urlTemplate:
              ...
              Positioned(
                ...
                child: Chip(
                  avatar: Icon(
                    s.freshFix ? Icons.gps_fixed : Icons.gps_not_fixed,
                    size: 18,
                  ),
                  label: Text(
                    s.freshFix
                        ? 'GPS ±${s.accuracy.round()} m'
                        : 'Chưa có GPS tốt',
                  ),
                ),
              ),
      ...
      const Padding(
        ...
        child: Text(
          'Nhấn giữ bản đồ để chọn điểm đến.',
      ...
      panel([
        ...
        if (s.route != null)
          Text(
            '${(s.route!.distance / 1000).toStringAsFixed(2)} km  •  ${(s.route!.seconds / 60).ceil()} phút  •  Đi bộ',
          ),
        if (s.route != null)
          Text(
            'Tuyến lưu: ${s.route!.created.toLocal().toString().substring(0, 16)}',
          ...
          Semantics(
            ...
            child: Text(
              s.journey == JourneyState.arrived
                  ...
                  ? 'Mất kết nối kính'
                  : !s.onRoute
                  ? 'Lệch tuyến — đang tìm lại đường'
                  : '${s.progress?.distanceToStep.round() ?? 0} m • ${s.instruction?.text ?? 'Đang định vị'}',
              style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w700),
            ),
          ),
          if (s.progress != null)
            Text('Còn ${s.progress!.remaining.round()} m theo tuyến'),
          if (s.syncPending && !s.demo)
            const Text(
              'Đang chờ kính xác nhận trạng thái…',
              style: TextStyle(color: Colors.deepOrange),
            ),
        ],
        const SizedBox(height: 18),
        if (!s.active && s.journey != JourneyState.paused)
          FilledButton.icon(
        ...
        if (s.active ||
          ...
          TextButton(
            ...
            child: const Text('Kết thúc hành trình'),
          ),
        if (s.route != null)
          ExpansionTile(
            title: const Text('Các bước chỉ dẫn'),
            tilePadding: EdgeInsets.zero,
            children: s.route!.steps
                .map(
                  (step) => ListTile(
                    leading: Icon(
      ...
      Text(
        s.demo
            ? 'Bật/tắt mô phỏng trong Cài đặt.'
            : 'Cảnh báo vật cản được xử lý và phát trực tiếp trên kính.',
      ...
      children: [
        ...
        panel([
          ...
          Center(
            child: Text(
              s.demo || s.status['demo'] == true
                  ? 'Tất cả số liệu dưới đây là giả lập'
                  : s.link.endpoint,
            ),
          ),
          const SizedBox(height: 18),
            ...
            onPressed: s.demo
                ...
                : () => s.perform(() async {
                    if (s.link.connected) {
                      ...
                      if (!s.link.connected)
                        s.notice =
                            'Chưa kết nối được. App sẽ tự thử lại; kiểm tra hotspot, địa chỉ và token.';
                    }
                  }),
            icon: Icon(s.link.connected ? Icons.link_off : Icons.link),
        ...
        Wrap(
          ...
          runSpacing: 12,
          children: [
            metric('Pin', value('battery_percent', '%'), Icons.battery_5_bar),
            metric('Nhiệt độ Pi', value('cpu_temp', ' °C'), Icons.thermostat),
            metric('YOLO', value('yolo_fps', ' FPS'), Icons.speed),
            metric(
              'Sai số GPS',
              s.freshFix ? '${s.accuracy.round()} m' : 'Chưa xác định',
              Icons.gps_fixed,
            ),
          ],
        ),
            ...
            (e) => ListTile(
              ...
              trailing: Text(
                !good
                    ...
                    : s.status[e.key] == true
                    ? 'Hoạt động'
                    : s.status[e.key] == false
                    ? 'Lỗi cảm biến'
                    : 'Chưa xác định',
                style: TextStyle(
                  color: good && s.status[e.key] == true
                      ? const Color(0xFF175C45)
                      : Colors.deepOrange,
                ),
              ),
            ),
        ...
        const SizedBox(height: 16),
        const Text(
          'Bật hotspot trong cài đặt điện thoại và cho Pi kết nối cùng mạng. App không tự bật hotspot. Nếu tên secondsight.local không phân giải, nhập IP LAN của Pi trong Cài đặt.',
        ),
        if (s.notice != null)
          Padding(
  ...
  Widget metric(String title, String value, IconData icon) => SizedBox(
    ...
    child: panel([
      Icon(icon, color: const Color(0xFF175C45)),
      const SizedBox(height: 8),
      Text(
        value,
        style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold),
  ...
  Widget logsPage() => ListView(
    ...
    children: [
      ...
      const SizedBox(height: 10),
      const Text(
        'Lưu tối đa 300 sự kiện trên thiết bị. Không ghi video, token hoặc toàn bộ vị trí GPS vào nhật ký.',
      ),
      Row(
        children: [
          TextButton.icon(
            onPressed: () => s.perform(
              () => shareText(
                const JsonEncoder.withIndent('  ').convert(s.events),
              ),
            ),
      ...
      ...s.events.map(
        (e) => Card(
          child: ListTile(
            leading: Icon(
              e['type'] == 'hazard' ? Icons.warning_amber : Icons.history,
            ),
            title: Text('${e['text']}'),
            subtitle: Text('${e['time']} • ${e['type']}'),
          ),
        ),
      ),
    ],
...
class _ConnectionSection extends StatelessWidget {
  ...
  final ValueChanged<bool> onAllowInsecureChanged;

  const _ConnectionSection({
    required this.s,
    required this.endpoint,
    required this.token,
    required this.onAllowInsecureChanged,
  });
    ...
    return Column(
      ...
      children: [
        ...
        TextField(
          ...
          autocorrect: false,
          decoration: const InputDecoration(
            labelText: 'Địa chỉ Pi',
            hintText: 'https://secondsight.local:8765',
          ),
        ),
        const SizedBox(height: 12),
        TextField(
        ...
        SwitchListTile.adaptive(
          contentPadding: EdgeInsets.zero,
          title: const Text('Cho phép HTTP LAN thử nghiệm'),
          subtitle: const Text(
            'Không mã hóa. Chỉ dùng mạng hotspot riêng tin cậy. Bản triển khai dùng HTTPS/WSS.',
          ),
          value: s.link.allowInsecure,
          onChanged: onAllowInsecureChanged,
        ),
...
class _MapContactSection extends StatelessWidget {
  final TextEditingController apiKey;
  final TextEditingController phone;

  const _MapContactSection({required this.apiKey, required this.phone});
...
class _PreferencesSection extends StatelessWidget {
  final AppState s;
  final ValueChanged<String?> onWarningLevelChanged;
  ...
  const _PreferencesSection({
    required this.s,
    required this.onWarningLevelChanged,
    ...
    return Column(
      ...
      children: [
        DropdownButtonFormField<String>(
          ...
          onChanged: onWarningLevelChanged,
        ),
        const SizedBox(height: 16),
        Text('Âm lượng: ${(s.volume * 100).round()}%'),
        Slider(
          value: s.volume,
          divisions: 10,
        ...
        SwitchListTile.adaptive(
          contentPadding: EdgeInsets.zero,
          value: s.vibration,
          title: const Text('Rung khi có cảnh báo'),
  ...
  Widget build(BuildContext context) => ListView(
    ...
    children: [
      ...
      SwitchListTile.adaptive(
        ...
        subtitle: const Text(
          'Thử tuyến, chỉ dẫn và cảnh báo mà không cần Pi hoặc API key.',
        ),
        value: s.demo,
      ...
      _ConnectionSection(
        ...
        endpoint: endpoint,
        token: token,
        onAllowInsecureChanged: (v) => setState(() => s.link.allowInsecure = v),
      ),
      _MapContactSection(apiKey: apiKey, phone: phone),
      _PreferencesSection(
        s: s,
        onWarningLevelChanged: (v) => setState(() => s.warningLevel = v!),
        onVolumeChanged: (v) => setState(() => s.volume = v),
        onSpeechRateChanged: (v) => setState(() => s.speechRate = v),
        onVibrationChanged: (v) => setState(() => s.vibration = v),
      ),
      FilledButton.icon(
        onPressed: () => s.perform(() async {
          final nextEndpoint = endpoint.text.trim(),
              nextToken = token.text.trim();
      ...
      if (saved)
        const Padding(
          ...
          child: Text(
            'Đã lưu trên điện thoại. Cấu hình kính sẽ đồng bộ khi kết nối.',
          ),
        ),
      const SizedBox(height: 12),
      OutlinedButton(
      ...
      TextButton(
        ...
        child: const Text('Mở quyền ứng dụng'),
      ),
      const SizedBox(height: 18),
      const Text(
        'Tuyến lưu dùng được khi mất mạng ngắn hạn; tìm địa điểm, tính lại tuyến và tải ô bản đồ cần Internet. Kính phải tự xử lý cảnh báo cục bộ khi mất kết nối.',
      ),
